(function($, window, undefined){
    ...
    function loadRandomPassage() {
        ...
        $.ajax({
            url: "http://labs.bible.org/api/?passage="+psg,
            data: {type:'json', callback: 'displayPassages', formatting:'para' },
            cache: false,
            dataType: "jsonp"
        });
    }
...
var displayPassages = function(passages) {
    var that = this;
    ...
    for (var i = 0; i < plength; i++) {
        ...
        if (p.bookname != curBook && p.chapter != curChapter) {
            // The passage needs to be started
            $el = $('<section>');
